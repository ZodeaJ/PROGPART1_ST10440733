...
namespace CyberSecurityChatBot
{
        ...
        private static readonly Dictionary<string, List<string>> tips = new Dictionary<string, List<string>>
    {
        ...
        public void DisplayRandomTip(string topic)
        {
            if (tips.ContainsKey(topic))
            {
                string randomTip = tips[topic][rng.Next(tips[topic].Count)];
                Console.ForegroundColor = ConsoleColor.Yellow;
                //Specify which tip it is for.
                Console.WriteLine($"{ToTitleCase(topic)} Tip: {randomTip}");
                Console.ResetColor();
            }
        }
